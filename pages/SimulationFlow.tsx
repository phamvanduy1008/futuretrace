...
const SimulationFlow: React.FC = () => {
  ...
  const [error, setError] = useState<{ message: string; type: 'AUTH' | 'NETWORK' | 'LOCAL_CONFIG' | 'GENERAL' | 'OVERLOADED' | 'RATE_LIMIT' } | null>(null);
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 relative z-10">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 ${
                  error.type === 'OVERLOADED' ? 'bg-amber-50 text-amber-600' : 'bg-rose-50 text-rose-600'
                }`}>
                  <IconMapper name={
                    error.type === 'AUTH' ? 'toll' :
                    error.type === 'RATE_LIMIT' ? 'block' :
                    error.type === 'OVERLOADED' ? 'hourglass_empty' : 'warning'
                  } className="text-3xl font-bold" />
                </div>
                <div className="flex-1">
                  <h3 className={`text-sm font-black uppercase tracking-widest mb-1 ${
                    error.type === 'OVERLOADED' ? 'text-amber-900' : 'text-rose-900'
                  }`}>
                    {error.type === 'AUTH' ? 'Không đủ token' :
                     error.type === 'RATE_LIMIT' ? 'Hết lượt sử dụng AI' :
                     error.type === 'OVERLOADED' ? 'Hệ thống AI quá tải' : 'Cảnh báo hệ thống'}
                  </h3>
