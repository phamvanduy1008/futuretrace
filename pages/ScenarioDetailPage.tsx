...
const ScenarioDetailPage: React.FC = () => {
  ...
  const fetchProgress = async (scenarioId: string) => {
    try {
      ...
      const response = await fetch(`${apiBase}/premium/progress/by-scenario/${scenarioId}`, {
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans relative">
      ...
      <main className="flex-1 max-w-[1200px] mx-auto w-full px-4 sm:px-6 py-10 sm:py-20">
        ...
        <header className="mb-16 sm:mb-24">
          <div className="flex items-center gap-3 text-[10px] font-black text-blue-600 uppercase tracking-[0.4em] mb-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
            <div className="max-w-4xl">
              ...
              <motion.p 
                ...
                className="text-lg sm:text-xl lg:text-2xl text-slate-500 font-medium italic leading-relaxed max-w-2xl"
              >
          ...
          <div className="lg:col-span-7 space-y-8 lg:space-y-12">
              ...
              {[
                ...
                <div key={i} className="bg-white p-6 sm:p-8 lg:p-10 rounded-[2rem] sm:rounded-[2.5rem] border border-slate-100 shadow-[0_15px_30px_-5px_rgba(0,0,0,0.05)] hover:shadow-[0_25px_50px_-10px_rgba(0,0,0,0.08)] transition-all text-center">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-inner border border-slate-100">
                    <span className={`material-symbols-outlined ${m.color} text-2xl sm:text-3xl`}>{m.icon}</span>
                  </div>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8">
              {scenario.deepAnalysis?.swot.map((item: any, i: number) => (
                ...
                >
                  <span className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest inline-block mb-6 border ${
                    item.type === 'S' || item.type === 'Strengths' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
            ...
            {(state?.fromCommunity || isSaved) && (
              <section className="bg-white/60 backdrop-blur-xl p-8 sm:p-10 lg:p-12 rounded-[2.5rem] sm:rounded-[4rem] border border-white/80 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.03)] mt-8 relative overflow-hidden">
                 ...
                 <div className="flex items-center justify-between mb-10">
                   <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-900 flex items-center gap-4">
                     <span className="material-symbols-outlined text-blue-600 bg-blue-50/50 p-2 rounded-xl text-xl">forum</span>
                 ...
                 <div className="flex gap-5 mb-10 pb-10 border-b border-slate-100/60">
                    ...
                    <form onSubmit={handlePostComment} className="flex-1">
                      ...
                      <div className="flex justify-end mt-3">
                        ...
                        >
                          GỬI Ý KIẾN <span className="material-symbols-outlined text-sm group-hover:translate-x-1 transition-transform">arrow_forward</span>
                        </button>
                      </div>
                    </form>
                 </div>
                   ...
                   <div className="space-y-6">
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="bg-slate-50/40 hover:bg-white hover:shadow-xl hover:shadow-slate-200/20 transition-all p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-100/50 relative group">
                                ...
                                >
                                  <span className={`material-symbols-outlined text-lg ${comment.isLiked ? 'fill-rose-500' : ''}`}>favorite</span>
                    ...
                    {comments.length === 0 && (
                      <div className="py-12 text-center flex flex-col items-center gap-4">
                        ...
                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">Hệ thống chưa nhận được bình luận nào.</p>
                      </div>
                    )}
                   </div>
                 )}
          ...
          <div className="lg:col-span-5 space-y-8 lg:space-y-12">
            <section className="bg-slate-950 p-10 sm:p-12 lg:p-14 rounded-[2.5rem] sm:rounded-[3.5rem] text-white shadow-[0_40px_80px_-20px_rgba(0,0,0,0.4)] relative overflow-hidden">
               <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none">
                  <span className="material-symbols-outlined text-[150px] sm:text-[200px] text-white">sprint</span>
               </div>
               <h3 className="text-[10px] sm:text-xs font-black uppercase tracking-[0.4em] text-blue-400 mb-10 sm:mb-14 relative z-10">Chiến thuật cơ bản</h3>
                 ...
                 {scenario.deepAnalysis?.sprint90.map((phase: any, i: number) => (
                   <div key={i} className="flex gap-8 sm:gap-10 group">
                      ...
                      <div className="pb-4 sm:pb-6">
                        <h4 className="text-[11px] sm:text-sm font-black uppercase tracking-[0.2em] text-white mb-6 sm:mb-8">
                          {phase.phase}
                        </h4>
            ...
            <div className="flex flex-col gap-4 sm:gap-6 pt-2">
               ...
               {!(state.fromCommunity && isOwner) && (
                 existingProgress ? (
                   <motion.button 
                     ...
                     className="w-full py-6 bg-gradient-to-br from-blue-500 to-blue-700 text-white font-black text-[10px] uppercase tracking-[0.4em] rounded-[1.5rem] sm:rounded-[2rem] shadow-[0_20px_40px_-10px_rgba(37,99,235,0.2)] flex items-center justify-center gap-4 relative overflow-hidden group"
                   >
                     VÀO TIẾN TRÌNH <span className="material-symbols-outlined text-xl">trending_up</span>
                   </motion.button>
                 ) : (
                   <motion.button 
                     ...
                     className="w-full py-6 bg-gradient-to-br from-amber-400 to-amber-600 text-white font-black text-[10px] uppercase tracking-[0.4em] rounded-[1.5rem] sm:rounded-[2rem] shadow-[0_20px_40px_-10px_rgba(245,158,11,0.2)] flex items-center justify-center gap-4 relative overflow-hidden group"
                   >
               ...
               {!state.fromCommunity && (
                 <button onClick={() => setIsModalOpen(true)} className="w-full py-6 bg-slate-900 text-white font-black text-[10px] uppercase tracking-[0.4em] rounded-[1.5rem] sm:rounded-[2rem] hover:bg-blue-600 transition-all shadow-xl flex items-center justify-center gap-4 group">
               ...
               {state.fromCommunity && isOwner && (
                 <button onClick={handleDeletePost} className="w-full py-6 bg-rose-50 border border-rose-100 text-rose-600 font-black text-[10px] uppercase tracking-[0.4em] rounded-[1.5rem] sm:rounded-[2rem] hover:bg-rose-600 hover:text-white transition-all shadow-sm flex items-center justify-center gap-4">
                   XÓA BÀI ĐĂNG <span className="material-symbols-outlined text-xl">delete</span>
                 </button>
               )}
               
               <button onClick={() => navigate(state.fromCommunity ? '/community' : '/history')} className="w-full py-6 bg-white border border-slate-200 text-slate-400 font-black text-[10px] uppercase tracking-[0.4em] rounded-[1.5rem] sm:rounded-[2rem] hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm text-center">
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 30 }} className="relative w-full max-w-[480px] bg-white rounded-[3rem] p-10 sm:p-14 shadow-[0_80px_120px_-40px_rgba(0,0,0,0.4)] border border-slate-100 overflow-hidden">
              <div className="text-center mb-10">
                ...
                <p className="text-slate-500 text-xs font-medium">Bạn muốn hoàn thành mục tiêu này trong bao lâu?</p>
              </div>
              
              <div className="space-y-8 mb-10">
                <div className="flex flex-col gap-4">
                  <div className="flex justify-between items-center px-2">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Thời gian (Tháng)</label>
              ...
              <div className="flex flex-col gap-3">
                <button 
                  ...
                  className="w-full py-5 bg-slate-900 text-white font-black text-[11px] uppercase tracking-[0.4em] rounded-[1.5rem] hover:bg-blue-600 shadow-xl transition-all flex items-center justify-center gap-3"
                >
                  XÁC NHẬN & PHÂN TÍCH <span className="material-symbols-outlined text-lg">bolt</span>
                </button>
                <button 
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 30 }} className="relative w-full max-w-[500px] bg-white rounded-[3rem] p-10 sm:p-14 shadow-[0_80px_120px_-40px_rgba(0,0,0,0.4)] border border-slate-100 overflow-hidden">
              {isSaved ? (
                <div className="py-10 text-center space-y-6">
                  <div className="w-24 h-24 bg-emerald-50 text-emerald-600 rounded-[2rem] flex items-center justify-center mx-auto shadow-xl border border-emerald-100"><span className="material-symbols-outlined text-5xl font-bold">verified</span></div>
                  <h3 className="text-2xl font-black text-slate-900 font-display uppercase tracking-tighter">Thành công!</h3>
                  <p className="text-slate-500 text-sm font-medium">Báo cáo của bạn đã được xuất bản an toàn.</p>
                </div>
              ) : (
                <form onSubmit={handleSaveSubmit} className="space-y-10 relative z-10">
                  <div className="text-center mb-10"><h3 className="text-2xl font-black text-slate-900 font-display uppercase tracking-tighter mb-3">Xuất bản báo cáo</h3><p className="text-slate-400 text-[9px] font-black uppercase tracking-[0.3em]">Hệ thống bảo mật dữ liệu AES-256</p></div>
                  <div className="space-y-8">
                    <div className="flex flex-col gap-3"><label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] ml-2">Tên báo cáo chính thức</label><input required value={saveForm.title} onChange={(e) => setSaveForm({...saveForm, title: e.target.value})} className="w-full h-14 px-6 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:bg-white focus:border-blue-600 transition-all font-bold text-slate-900 shadow-inner" placeholder="Ví dụ: Lộ trình sự nghiệp 2024" /></div>
                    <label className="flex items-center justify-between cursor-pointer p-5 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-white transition-all group"><div className="flex items-center gap-4"><span className="material-symbols-outlined text-xl text-amber-500">visibility_off</span><span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Đăng ẩn danh</span></div><input type="checkbox" checked={saveForm.isAnonymous} onChange={(e) => setSaveForm({...saveForm, isAnonymous: e.target.checked})} className="w-6 h-6 rounded-lg text-amber-500 border-slate-200" /></label>
                  </div>
                  <button type="submit" disabled={isSaving} className="w-full py-6 bg-slate-900 text-white font-black text-[11px] uppercase tracking-[0.4em] rounded-[1.5rem] hover:bg-blue-600 shadow-xl transition-all disabled:opacity-50 flex items-center justify-center gap-3">{isSaving ? "Đang mã hóa..." : "Xác nhận & Xuất bản"}</button>
                </form>
              )}
            </motion.div>
          </div>
