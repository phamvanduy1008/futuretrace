...
const ScenarioDetailPage: React.FC = () => {
  ...
  const fetchProgress = async (scenarioId: string) => {
    try {
      ...
      const apiBase =
        ...
        "https://futuretrace-server.onrender.com";
      const response = await fetch(
        `${apiBase}/api/premium/progress/by-scenario/${scenarioId}`,
        {
  ...
  const handleConfirmTimeframe = () => {
    ...
    navigate("/premium-analysis", {
      state: { scenario, context: state.context, timeframe },
    });
  };
  ...
  const handleDeletePost = async () => {
    if (
      !id ||
      !window.confirm("Bạn có chắc chắn muốn xóa bài viết này khỏi cộng đồng?")
    )
      return;
    try {
  ...
  const isOwner =
    currentUser &&
    ((state.fromCommunity &&
      String(scenarioUserId) === String(currentUser.id)) ||
      ...
      <main className="flex-1 max-w-[1200px] mx-auto w-full px-4 sm:px-6 py-10 sm:py-20">
        ...
        <header className="mb-16 sm:mb-24">
          <div className="flex items-center gap-3 text-[10px] font-black text-blue-600 uppercase tracking-widest mb-8">
            <IconMapper name="science" className=" text-[16px]" /> Nghiên cứu
            chuyên sâu
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
            <div className="max-w-4xl">
              ...
              <motion.p
                ...
                className="text-lg sm:text-xl lg:text-2xl text-slate-600 font-medium italic leading-relaxed max-w-2xl text-justify hyphens-auto"
              >
              ...
              {[
                ...
                >
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-inner border border-slate-100">
                    <IconMapper
                      name={m.icon}
                      className={` ${m.color} text-2xl sm:text-3xl`}
                    />
                  </div>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8">
              {scenario.deepAnalysis?.swot.map((item: any, i: number) => (
                ...
                >
                  ...
                  <p className="text-base sm:text-lg font-bold text-slate-800 leading-relaxed italic text-justify">
                    "{item.value}"
                  </p>
                </motion.div>
              ))}
              ...
              <section className="bg-white/60 backdrop-blur-xl p-8 sm:p-10 lg:p-12 rounded-[2.5rem] sm:rounded-[4rem] border-2 border-white/80 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.03)] hover:border-blue-600/50 hover:ring-4 hover:ring-blue-600/10 transition-all mt-8 relative overflow-hidden">
                <div className="flex items-center justify-between mb-10">
                  <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-900 flex items-center gap-4">
                    <IconMapper
                      name="forum"
                      className=" text-blue-600 bg-blue-50/50 p-2 rounded-xl text-xl"
                    />
                ...
                <div className="flex gap-5 mb-10 pb-10 border-b border-slate-100/60">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-slate-900 flex items-center justify-center shrink-0 border-2 border-white shadow-lg font-black text-white text-[10px]">
                    ME
                  </div>
                        ...
                        >
                          <div className="shrink-0 relative pt-1">
                            <img
                              ...
                              className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl border border-slate-100 shadow-sm"
                            ...
                            <div className="bg-slate-50/40 hover:bg-white hover:shadow-xl hover:shadow-slate-200/20 transition-all p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-100/50 relative group">
                              <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-2">
                                  <span className="text-[10px] sm:text-[11px] font-black text-slate-900 uppercase tracking-widest">
                                    {comment.authorName}
                                  </span>
                                ...
                                <p className="text-slate-600 text-sm sm:text-base font-medium leading-relaxed italic pr-12">
                                  "{comment.content}"
                                </p>
                                ...
                                <button
                                  onClick={async () => {
                                    try {
                                      const res =
                                        await communityService.toggleCommentLike(
                                          id!,
                                          comment.id,
                                        );
                                      setComments((prev) =>
                                ...
                                >
                                  <IconMapper
                                    name="favorite"
                                    className={` text-lg ${comment.isLiked ? "fill-rose-500" : ""}`}
                                  />
                    ...
                    {comments.length === 0 && (
                      <div className="py-12 text-center flex flex-col items-center gap-4">
                        ...
                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                          Hệ thống chưa nhận được bình luận nào.
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="lg:col-span-5 space-y-8 lg:space-y-12">
            <section className="bg-slate-950 p-10 sm:p-12 lg:p-14 rounded-[2.5rem] sm:rounded-[3.5rem] text-white shadow-[0_40px_80px_-20px_rgba(0,0,0,0.4)] relative overflow-hidden">
              <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none">
                <IconMapper
                  name="sprint"
                  className=" text-[150px] sm:text-[200px] text-white"
                />
              </div>
              <h3 className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-blue-400 mb-10 sm:mb-14 relative z-10">
                Chiến thuật cơ bản
              </h3>
              <div className="space-y-10 sm:space-y-14 relative z-10">
                {scenario.deepAnalysis?.sprint90.map(
                  (phase: any, i: number) => (
                    <div key={i} className="flex gap-8 sm:gap-10 group">
                      <div className="flex flex-col items-center shrink-0">
                        ...
                        {i < 2 && (
                          <div className="w-px h-full bg-slate-800 mt-6 shadow-[0_0_10px_rgba(37,99,235,0.2)]"></div>
                        )}
                      </div>
                      <div className="pb-4 sm:pb-6">
                        ...
                        <ul className="space-y-4 sm:space-y-6">
                          {phase.tasks.map((task: string, ti: number) => (
                            <li
                              key={ti}
                              className="flex items-start gap-4 text-slate-400 text-sm font-medium leading-relaxed"
                            >
            ...
            <div className="flex flex-col gap-4 sm:gap-6 pt-2">
              ...
              {!(state.fromCommunity && isOwner) &&
                (existingProgress ? (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() =>
                      navigate(`/progress?id=${existingProgress.id}`)
                    }
                  ...
                  >
                    VÀO TIẾN TRÌNH{" "}
                    <IconMapper name="trending_up" className=" text-xl" />
                  </motion.button>
                ) : (
                  <motion.button
                    whileHover={{
                      scale: 1.02,
                      boxShadow: "0 30px 60px -15px rgba(245,158,11,0.25)",
                    }}
                    whileTap={{ scale: 0.98 }}
              ...
              {!state.fromCommunity && (
                <button
                  ...
                  className="w-full py-6 bg-slate-900 text-white font-black text-[10px] uppercase tracking-widest rounded-[1.5rem] sm:rounded-[2rem] hover:bg-blue-600 transition-all shadow-xl flex items-center justify-center gap-4 group"
                >
                  XUẤT BẢN{" "}
                  <IconMapper
                    name="share"
              ...
              {state.fromCommunity && isOwner && (
                <button
                  ...
                  className="w-full py-6 bg-rose-50 border border-rose-100 text-rose-600 font-black text-[10px] uppercase tracking-widest rounded-[1.5rem] sm:rounded-[2rem] hover:bg-rose-600 hover:text-white transition-all shadow-sm flex items-center justify-center gap-4"
                >
              ...
              <button
                onClick={() =>
                  navigate(state.fromCommunity ? "/community" : "/history")
                }
                className="w-full py-6 bg-white border border-slate-200 text-slate-400 font-black text-[10px] uppercase tracking-widest rounded-[1.5rem] sm:rounded-[2rem] hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm text-center"
              >
                {state.fromCommunity
                  ? "QUAY LẠI CỘNG ĐỒNG"
                  : "QUAY LẠI LỊCH SỬ"}
        ...
        {isTimeframeModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-[480px] bg-white rounded-[3rem] p-10 sm:p-14 shadow-[0_80px_120px_-40px_rgba(0,0,0,0.4)] border border-slate-100 overflow-hidden"
            >
              <div className="text-center mb-10">
                ...
                <p className="text-slate-600 text-xs font-medium">
                  Bạn muốn hoàn thành mục tiêu này trong bao lâu?
                </p>
              </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !isSaving && !isSaved && setIsModalOpen(false)}
              className="absolute inset-0 bg-slate-900/70 backdrop-blur-md"
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.9, y: 30 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 30 }}
              className="relative w-full max-w-[500px] bg-white rounded-[3rem] p-10 sm:p-14 shadow-[0_80px_120px_-40px_rgba(0,0,0,0.4)] border border-slate-100 overflow-hidden"
            >
              {isSaved ? (
                <div className="py-10 text-center space-y-6">
                  <div className="w-24 h-24 bg-emerald-50 text-emerald-600 rounded-[2rem] flex items-center justify-center mx-auto shadow-xl border border-emerald-100">
                    <IconMapper
                      name="verified"
                      className=" text-5xl font-bold"
                    />
                  </div>
                  <h3 className="text-2xl font-black text-slate-900 font-display uppercase tracking-tighter">
                    Thành công!
                  </h3>
                  <p className="text-slate-600 text-sm font-medium">
                    Báo cáo của bạn đã được xuất bản an toàn.
                  </p>
                </div>
              ) : (
                <form
                  ...
                  <div className="space-y-8">
                    <div className="flex flex-col gap-3">
                      ...
                      <input
                        ...
                        className="w-full h-14 px-6 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:bg-white focus:border-blue-600 transition-all font-bold text-slate-900 shadow-inner"
                        placeholder="Ví dụ: Lộ trình sự nghiệp 2024"
                      />
                    </div>
                    <label className="flex items-center justify-between cursor-pointer p-5 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-white transition-all group">
                      ...
                      <input
                        ...
                        className="w-6 h-6 rounded-lg text-amber-500 border-slate-200"
                      />
                    </label>
                  </div>
                  <button
